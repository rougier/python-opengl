...
void main() {
    ...
    // Fading t line start and end (optional)
    if (linelength - center < 1.5*thickness)
        alpha = 1.0 - (1.5*thickness - (linelength - center))/thickness;
    else if (center < 1.5*thickness)
        alpha = 1.0 - (1.5*thickness - center)/thickness;
    ...
    if (linelength - center < thickness/2.0)
        ...
        if (length(v_uv - vec2(center,0.0)) < .9*thickness)
            d = marker_spade(v_uv - vec2(center,0.0), .9*thickness);
        else
            discard;
    }
...
n = 512
T = np.linspace(0, 5*2*np.pi, n, dtype=np.float32)
R = np.linspace(64, 240, n, dtype=np.float32)
...
app.run(framerate=60, framecount=150)
